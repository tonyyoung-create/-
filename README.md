...
- 我已在本機使用專案內的 CSV 執行一次快速 demo（跳過 API 以避免 SSL/網路問題）：訓練 + 驗證 + 預測下一小時。
- 資料範圍：2025-12-01 到 2025-12-17，包含污染物 PM2.5, PM10, O3, CO, SO2, NO2。
- 部分結果摘要（預測下一小時與標準比較）：
  - PM2.5 預測 ~20.9 μg/m3（超過 WHO 24h 建議 15 μg/m3 → 顯示警告，建議敏感族群減少戶外活動）
  - PM10 預測 ~48.8 μg/m3（超過 WHO 24h 建議 45 μg/m3 → 顯示警告）
  - 其他污染物（O3, CO, SO2, NO2）在該次 demo 中為「符合主要參考標準」。

> 注意：demo 使用的是本地 CSV，實際部署可啟用 API 取得即時資料；若部署在 Streamlit Cloud，請確認憑證（或改用 certifi）以避免 SSL 問題。